...
def non_decrease_subseqs(s):
    ...
    """
    def subseq_helper(s, prev):   # assume that the input list contains no negative elements.
        if not s:
            return [[]]
        elif s[0] < prev:
    ...
    return subseq_helper(s, 0)   # prev is used to compare with elements in S.
...
def merge(incr_a, incr_b):
    ...
    "*** YOUR CODE HERE ***"
    while not (next_b == None and next_a == None):
        if next_a < next_b :
            yield next_a
            ...
            yield next_a
            next_a, next_b = next(iter_a, None), next(iter_b, None)
    # incre_a is not empty 
    while next_a:
        yield next_a
        next_a = next(iter_a, None)  
    # incre_b is not empty
    while next_b:
        yield next_b
        next_b = next(iter_b, None) 
...
def trade(first, second):
    ...
    while m <= len(first) and n <= len(second) and not equal_prefix():  #第三个判断条件没想到
...
def insert(link, value, index):
    ...
    # if index == 2:
    #     link.rest.rest = Link(value, link.rest.rest)
    
    i = 0
    curr = link
    while curr is not Link.empty:
        if i == index:
            # key point: how to clone a linked list?
            curr_copy = Link(curr.first, curr.rest)
            curr.first = value
            curr.rest = curr_copy
            #curr.rest = curr
            #curr.first = value
...
def deep_len(lnk):
    ...
    if lnk is Link.empty:
        return 0
    elif isinstance(lnk, int):   #这个条件句没想到，写成了not isinstance(lnk, Link) and lnk.rest == Link.empty
        return 1
    else:
