...
class Place:  #can only have one ant, more than one bees.
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # reflective damage:
         # damage amount = amount + self.damage(if the ant's health has drpped to 0)
        if self.health > amount:
            ref_damage = amount
        else:
            ref_damage = amount + self.damage
         # For each bee in the place, damage them with the total amount 
          # (by calling the appropriate reduce_health method for each bee.)
        for b in self.place.bees[:]:    # key point: copy the list first!!
            b.reduce_health(ref_damage)
        # reduce the fireant health by the amount: should be the last step!
        super().reduce_health(amount)
        

        # remove the insect from its place if its health <= 0
        # END Problem 5
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        #First, check if it is chewing; 
        if self.chew_countdown:
        #if so, decrement its chew_countdown. 
        ...
        #Otherwise, eat a random Bee in its place by reducing the Bee's health to 0.
            ...
            chosen_bee.health = 0
            self.chew_countdown = self.chew_countdown
# END Problem 7
