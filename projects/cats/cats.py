...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    char_count = len(typed)/5
    time_in_minute = elapsed/60
    return char_count/time_in_minute
    # END PROBLEM 4
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    Arguments:
        ...
        takes in three arguments:(typed_word, word from valid words, limit)
        limit: a number
    ...
    if typed_word in valid_words:
        return typed_word
    # if the typed_word is not in valid_words:
    # look through the valid words and find the one that has the lowest difference from the typed word
    diff_list = [diff_function(typed_word, word, limit) for word in valid_words]
    # if the lowest difference > limit -- return typed_word
    if min(diff_list) > limit:
        return typed_word
    # otherwise, return the word from valid words that has the lowest difference from the typed one.
    else:
...
def feline_flips(start, goal, limit):
    ...
    Arguments:
        start: a starting word, a string representing a word
    ...
    # count the number of characters that must change.
    # If the strings are not of equal length, the difference in lengths is added to the total.
    # If the the number of characters that must change is greater than limit,--> return limit + 1
    if start == goal:
        return 0
    elif limit == 0:
        return 1
...
def time_per_word(words, times_per_player):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    time = [[player[i]-player[i-1] for i in range(1,len(player))] for player in times_per_player]
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    #for every word in words, compare whose time is shorter
    result = [[] for i in player_indices]
    for i in word_indices:
        times = [time(match, j, i) for j in player_indices]  #所有player打第i个字所用时间
        fastest_player = times.index[min(times)]             #用时最短的player的ID
        result[fastest_player].append(word_at(match, i))
    return result
    # END PROBLEM 10
